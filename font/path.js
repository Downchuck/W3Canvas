...
var CanvasRenderingContext2DFont_svg = {
 ...
 'load': function(family,svg) {
	...
	var i=0,ie=0,il=svg.length; var l='';
	il = svg.indexOf('</font');
	...
	*/
	...
	while(xi < il && 0 < (ie = svg.indexOf(tag,xi))) {	
		...
		var kern = [];
		if(g1) { g1=g1.split(','); for(i in g1) if(g1[i] in _.lettersn) for(ii in _.lettersn[g1[i]]) kern.push(_.lettersn[g1[i]][ii]); }
		if(u1) { u1=u1.split(','); for(i in u1) kern.push(u1); }

		var kerns = [];
		if(g2) { g2=g2.split(','); for(i in g2) if(g2[i] in _.lettersn) for(ii in _.lettersn[g2[i]]) kerns.push(_.lettersn[g2[i]][ii]); }
...
var CanvasRenderingContext2DPath = {
 ...
 'loadPath': function(path) {
	...
	var reg = ''; for(i in this._path) reg+=i;
	...
	while(i < pathl) {
		...
		var xes = path.substr(cur,end-cur).split(','); var is;
