...
(function () {
    var ComboBoxLayoutManager = function (textLayer) {
        ...
        var getTextWidth = function (text, font) {
            var w = Math.min(TEXTBOX_MAX_WIDTH, Math.max(TEXTBOX_MIN_WIDTH, measureText(text, font)));
            return w;
        };
        return {
    ...
    colorjack.controlFactory.ComboBox = function (textLayer) {
        ...
        var getFont = function () {
            if (!font) {
                throw new Error("colorjack.controlFactory.ComboBox.getFont(): Need to set font first");
            }
            return font;
        };
