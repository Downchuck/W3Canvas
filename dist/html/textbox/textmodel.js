...
colorjack.textbox.model.TextModel = function () {
    ...
    //---------------------------------------------------------------------------------------------------
    var getWordRange = function (container, offset) {
        var result = null;
        if (container === -1) {
            return result;
        }
        ...
        if (!colorjack.util.isWordSeparator(line.charAt(offset))) {
            ...
            for (var i = offset; i > 0; i--) {
                if (colorjack.util.isWordSeparator(line.charAt(i))) {
                    start = i + 1;
                    break;
                }
            }
            ...
            for (var u = offset; u < line.length; u++) {
                if (colorjack.util.isWordSeparator(line.charAt(u))) {
                    end = u;
                    break;
                }
            }
