...
colorjack.textbox.model.BasicModel = function () {
    ...
    var getLine = function (container) {
        if (container < 0 || container >= getLineCount()) {
            colorjack.debug.programmerPanic('Perhaps not a big deal.... getLine(): invalid line beyond current size: '
            ...
            for (i = 0; i < styles.length; i++) {
                ...
                if (styles[i].isGeneratedContent()) {
                    ...
                    if (fontStyle == "") { //no font style specified for this -> use the current (non-generated) one!
                        ...
                        var j = i - 1;
                        while (j >= 0 && styles[j].isGeneratedContent())
                            j--;
                        if (j >= 0)
                            fontStyle = styles[j].fontStyle;
                        else
                ...
                if (i + 1 < styles.length) {
                    ...
                    nextInd = content.length; //last fragment!
        ...
        for (i = 0; i < styles.length; i++) {
            if (styles[i].exOffset < exOffset) {
                ...
                if (i < styles.length - 1)
                    nextInd = styles[i + 1].exOffset;
                else
                    nextInd = result.length; //end of the new text //TODO: check this!
                if (nextInd >= offset) {
        ...
        for (i = 0; i < styles.length; i++) {
            if (styles[i].index < start) {
                ...
                if (i < styles.length - 1)
                    ...
                    nextInd = text.length; //length of the original text
                if (nextInd > end) {
...
colorjack.textbox.model.EditLineModel = function () {
    ...
    var getLineLength = function (container) {
        if (container === undefined) {
            colorjack.debug.programmerPanic("getLineLength(): Need container!");
        }
        var len = 0;
        var line = getLine(container);
        if (line) {
