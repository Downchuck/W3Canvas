...
var CanvasRenderingContext2DFont = {
    ...
    'loadFontStyle': function () {
        ...
        else if (this.style.fontWeight)
            this.lineWidth = (this.style.fontWeight * .01) * 5;
    ...
    'mozDrawText': function (textToDraw) {
        ...
        if (this.fontQueue.length) {
            ...
            if (isLoaded) {
                this.font.style = a[1];
                this._mozDrawText(this.font.value = a[0]);
            }
            else if (this.fontLoadTimeout > (a[2] += this.fontLoadWait))
                this.fontQueue.push([a[0], a[1], a[2]]);
        }
        if (this.fontQueue.length)
    ...
    'drawString': function (ctx, text, offset, maxlen) {
        ...
        for (i = offset; i < maxlen; i++) {
            ...
            if (code == 10 || code == 13)
                continue; // Non-printing characters
