...
export function BoxModelPainter() {
  ...
  var paintRoundedBoxGradient = function(ctx, box, gradient) { // could be phased out: use paintRoundedTextBox(ctx, box, gradientPattern)
    ctx.save();
  ...
  var paintRoundedBox = function(ctx, x, y, w, h, squareX, squareY) {
    ...
    if( rad_X > Math.min( w, h ) / 2 ) { rad_X = Math.min( w, h ) / 2; }
    if( rad_Y > Math.min( w, h ) / 2 ) { rad_Y = Math.min( w, h ) / 2; }
